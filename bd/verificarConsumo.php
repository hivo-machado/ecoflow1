<?php 
	...
	echo "Tempo Atual: ", $dataAtual, " Tempo dia anterior: ", $dataDiaAnterior, " Tempo mes anterior: ", $dataMesAnterior, "<br>";
  	...
  	while ($usuario = mysqli_fetch_object($usuarios) ){
		...
		//Leitura atual
		$unidadeMesAtualSelect = mysqli_query($con, "SELECT * FROM unidade WHERE idecoflow = '$usuario->id_unidade' AND tempo = '$dataAtual'ORDER BY tempo DESC, hora ASC LIMIT 1");
  		$unidadeAtual = mysqli_fetch_object($unidadeMesAtualSelect);

  		if($unidadeAtual != null){
  			
			//Leitura do dia anterior
			$unidadeDiaAnteriorSelect = mysqli_query($con, "SELECT * FROM unidade WHERE idecoflow = '$usuario->id_unidade' AND tempo = '$dataDiaAnterior'ORDER BY hora ASC LIMIT 1");
	  		$unidadeDiaAnterior = mysqli_fetch_object($unidadeDiaAnteriorSelect);	  		
	  		
	  		if($unidadeDiaAnterior != null){

	  			//Leitura no mes anterior
				$unidadeMesAnteriorSelect = mysqli_query($con, "SELECT * FROM unidade WHERE idecoflow = '$usuario->id_unidade' AND tempo <= '$dataMesAnterior' ORDER BY tempo DESC, hora ASC LIMIT 1");
	  		...
	  		if($consumoDia > $consumoMedio * RAZAO){
	  			...
	  			$unidades = mysqli_query($con, "SELECT * FROM unidade WHERE idecoflow = '$usuario->id_unidade' AND tempo = '$dataDiaAnterior' ORDER BY hora DESC");
